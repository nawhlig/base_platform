...
function Togo()
{
    ...
    React.useEffect(()=>{
        const fetchItemList = async () => {
            try {
                ...
                const response_helpcall = await API.get("helpcall/?na=토고");
    ...
    return(
        <>
            <div className='countryheader'>Togo</div>
            <div className='sosbox'>
                ...
                <Link exact to='/togomap'><button className='btn4'><img className="btnicon" src={map}/><br/>Map</button></Link>
            </div>
        </>
    )
