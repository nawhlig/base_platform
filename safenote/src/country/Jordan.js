...
function Jordan()
{
    ...
    React.useEffect(()=>{
        const fetchItemList = async () => {
            try {
                ...
                const response_helpcall = await API.get("helpcall/?na=요르단");
    ...
    return(
        <>
            <div className='countryheader'>Jordan</div>
            <div className='sosbox'>
                ...
                <Link exact to='/jordanmap'><button className='btn4'><img className="btnicon" src={map}/><br/>Map</button></Link>
            </div>
        </>
    )
