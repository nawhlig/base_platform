...
function Austria()
{
    ...
    React.useEffect(()=>{
        const fetchItemList = async () => {
            try {
                ...
                const response_helpcall = await API.get("helpcall/?na=오스트리아");
    ...
    return(
        <>
            <div className='countryheader'>Austria</div>
            <div className='sosbox'>
                ...
                <Link exact to='/austriamap'><button className='btn4'><img className="btnicon" src={map}/><br/>Map</button></Link>
            </div>
        </>
    )
