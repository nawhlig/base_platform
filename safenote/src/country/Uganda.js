...
function Uganda()
{
    ...
    React.useEffect(()=>{
        const fetchItemList = async () => {
            try {
                ...
                const response_helpcall = await API.get("helpcall/?na=우간다");
    ...
    return(
        <>
            <div className='countryheader'>Uganda</div>
            <div className='sosbox'>
                ...
                <Link exact to='/ugandamap'><button className='btn4'><img className="btnicon" src={map}/><br/>Map</button></Link>
            </div>
        </>
    )
