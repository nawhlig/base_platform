...
function NewZealand()
{
    ...
    React.useEffect(()=>{
        const fetchItemList = async () => {
            try {
                ...
                const response_helpcall = await API.get("helpcall/?na=뉴질랜드");
    ...
    return(
        <>
            <div className='countryheader'>NewZealand</div>
            <div className='sosbox'>
                ...
                <Link exact to='/newzealandmap'><button className='btn4'><img className="btnicon" src={map}/><br/>Map</button></Link>
            </div>
        </>
    )
