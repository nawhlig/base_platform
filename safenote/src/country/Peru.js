...
function Peru()
{
    ...
    React.useEffect(()=>{
        const fetchItemList = async () => {
            try {
                ...
                const response_helpcall = await API.get("helpcall/?na=페루");
    ...
    return(
        <>
            <div className='countryheader'>Peru</div>
            <div className='sosbox'>
                ...
                <Link exact to='/perumap'><button className='btn4'><img className="btnicon" src={map}/><br/>Map</button></Link>
            </div>
        </>
    )
