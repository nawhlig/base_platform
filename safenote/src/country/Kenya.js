...
function Kenya()
{
    ...
    React.useEffect(()=>{
        const fetchItemList = async () => {
            try {
                ...
                const response_helpcall = await API.get("helpcall/?na=케냐");
    ...
    return(
        <>
            <div className='countryheader'>Kenya</div>
            <div className='sosbox'>
                ...
                <Link exact to='/kenyamap'><button className='btn4'><img className="btnicon" src={map}/><br/>Map</button></Link>
            </div>
        </>
    )
