...
function Gambia()
{
    ...
    React.useEffect(()=>{
        const fetchItemList = async () => {
            try {
                ...
                const response_helpcall = await API.get("helpcall/?na=잠비아");
    ...
    return(
        <>
            <div className='countryheader'>Gambia</div>
            <div className='sosbox'>
                ...
                <Link exact to='/gambiamap'><button className='btn4'><img className="btnicon" src={map}/><br/>Map</button></Link>
            </div>
        </>
    )
