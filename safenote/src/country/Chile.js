...
function Chile()
{
    ...
    React.useEffect(()=>{
        const fetchItemList = async () => {
            try {
                ...
                const response_helpcall = await API.get("helpcall/?na=칠레");
    ...
    return(
        <>
            <div className='countryheader'>Chile</div>
            <div className='sosbox'>
                ...
                <Link exact to='/chilemap'><button className='btn4'><img className="btnicon" src={map}/><br/>Map</button></Link>
            </div>
        </>
    )
