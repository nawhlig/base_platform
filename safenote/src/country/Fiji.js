...
function Fiji()
{
    ...
    React.useEffect(()=>{
        const fetchItemList = async () => {
            try {
                ...
                const response_helpcall = await API.get("helpcall/?na=피지");
    ...
    return(
        <>
            <div className='countryheader'>Fiji</div>
            <div className='sosbox'>
                ...
                <Link exact to='/fijimap'><button className='btn4'><img className="btnicon" src={map}/><br/>Map</button></Link>
            </div>
        </>
    )
