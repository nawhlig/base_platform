...
function Egypt()
{
    ...
    React.useEffect(()=>{
        const fetchItemList = async () => {
            try {
                // 요청이 시작 될 때에는 error 와 itemlist 를 초기화하고
                setError(null);
                ...
                const response_helpcall = await API.get("helpcall/?na=이집트");
    ...
    return(
        <>
            <div className='countryheader'>Egypt</div>
            <div className='sosbox'>
                ...
                <Link exact to='/egyptmap'><button className='btn4'><img className="btnicon" src={map}/><br/>Map</button></Link>
            </div>
        </>
    )
