...
function France()
{
    ...
    React.useEffect(()=>{
        const fetchItemList = async () => {
            try {
                ...
                const response_helpcall = await API.get("helpcall/?na=프랑스");
    ...
    return(
        <>
            <div className='countryheader'>France</div>
            <div className='sosbox'>
                ...
                <Link exact to='/francemap'><button className='btn4'><img className="btnicon" src={map}/><br/>Map</button></Link>
            </div>
        </>
    )
