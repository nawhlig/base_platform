...
function Tuvalu()
{
    ...
    React.useEffect(()=>{
        const fetchItemList = async () => {
            try {
                ...
                const response_helpcall = await API.get("helpcall/?na=투발루");
    ...
    return(
        <>
            <div className='countryheader'>Tuvalu</div>
            <div className='sosbox'>
                ...
                <Link exact to='/tuvalumap'><button className='btn4'><img className="btnicon" src={map}/><br/>Map</button></Link>
            </div>
        </>
    )
