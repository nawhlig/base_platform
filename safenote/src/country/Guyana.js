...
function Guyana()
{
    ...
    React.useEffect(()=>{
        const fetchItemList = async () => {
            try {
                ...
                const response_helpcall = await API.get("helpcall/?na=가이아나");
    ...
    return(
        <>
            <div className='countryheader'>Guyana</div>
            <div className='sosbox'>
                ...
                <Link exact to='/guyanamap'><button className='btn4'><img className="btnicon" src={map}/><br/>Map</button></Link>
            </div>
        </>
    )
