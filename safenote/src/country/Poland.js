...
function Poland()
{
    ...
    React.useEffect(()=>{
        const fetchItemList = async () => {
            try {
                ...
                const response_helpcall = await API.get("helpcall/?na=폴란드");
    ...
    return(
        <>
            <div className='countryheader'>Poland</div>
            <div className='sosbox'>
                ...
                <Link exact to='/polandmap'><button className='btn4'><img className="btnicon" src={map}/><br/>Map</button></Link>
            </div>
        </>
    )
