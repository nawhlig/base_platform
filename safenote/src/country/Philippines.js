...
function Philippines()
{
    ...
    React.useEffect(()=>{
        const fetchItemList = async () => {
            try {
                ...
                const response_helpcall = await API.get("helpcall/?na=필리핀");
    ...
    return(
        <>
            <div className='countryheader'>Philippines</div>
            <div className='sosbox'>
                ...
                <Link exact to='/philippinesmap'><button className='btn4'><img className="btnicon" src={map}/><br/>Map</button></Link>
            </div>
        </>
    )
